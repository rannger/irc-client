...
NSString* kMessagePrefix=@"IRCMessagePrefix";
NSString* kMessageCommand=@"IRCMessageCommand";
NSString* kMessageArges=@"IRCMessageArges";
NSString* kMessageTail=@"IRCMessageTail";

static NSDictionary* ircDic=nil;
@implementation Message

+ (NSDictionary*)parseMessage:(NSString*)messageString;
{
    static NSString* regex=@"^(:(\\S+) )?(\\S+)( (?!:)(.+?))?( :(.+))?$";
    NSArray* array=[messageString arrayOfCaptureComponentsMatchedByRegex:regex];
    if ([array count]!=0) {
        
        NSArray* arr=[array lastObject];
        NSMutableDictionary* dic=[NSMutableDictionary dictionaryWithDictionary:@{kMessagePrefix: @(2),kMessageCommand:@(3),kMessageArges:@(5),kMessageTail:@(7)}];
        for (NSString* key in [dic allKeys]) {
            NSInteger index=[[dic objectForKey:key] integerValue];
            
            [dic setObject:[arr objectAtIndex:index] forKey:key];
        }
        
        NSString* replyFlag=[[self ircFlags] objectForKey:[dic objectForKey:kMessageCommand]];
        if (replyFlag!=nil) {
            [dic setObject:replyFlag forKey:kMessageCommand];
        }
        return dic;
    }
